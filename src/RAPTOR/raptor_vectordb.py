...
class RaptorVectorDB:
    def __init__(self, api_key :str , index_name : str, embd_model :str ,  data : list[Document]):
        load_dotenv()
    ...
    def store_docs(self, docs : list[Document]) -> None:
        if self.retriever and self.vectorstore:
            if isinstance(docs, list) and isinstance(docs[0], Document):
    ...
    def get_context(self, query : str , filter_key : str, filter_value : str) -> list[Document]:
        if self.retriever and self.vectorstore:
            if self.data[0].metadata.get(filter_key):
